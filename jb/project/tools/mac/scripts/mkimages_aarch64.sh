...
function create_jbr {
  ...
  *)
  ...
  JBR=${JBR_BASE_NAME}-osx-aarch64-b${build_number}
  ...
  #cp -a Frameworks/JavaNativeFoundation.framework ${JRE_HOME}/lib || do_exit $?
  ...
  echo Pach libawt dependency name: ${BASE_DIR}/jbr/Contents/Home/lib/libawt.dylib
  NAME_ORIG=@rpath/JavaNativeFoundation.framework/Versions/A/JavaNativeFoundation
  NAME_NEW=@loader_path/../../../../Frameworks/JavaNativeFoundation.framework/Versions/A/JavaNativeFoundation
  #otool -L ${BASE_DIR}/jbr/Contents/Home/lib/libawt.dylib
  install_name_tool -change $NAME_ORIG $NAME_NEW ${BASE_DIR}/jbr/Contents/Home/lib/libawt.dylib
  #otool -L ${BASE_DIR}/jbr/Contents/Home/lib/libawt.dylib
...
RELEASE_NAME=macosx-aarch64-normal-server-release
JBSDK=${JBRSDK_BASE_NAME}-osx-aarch64-b${build_number}
case "$bundle_type" in
  "jfx")
  ...
  "fd")
    ...
    RELEASE_NAME=macosx-aarch64-normal-server-fastdebug
    JBSDK=${JBRSDK_BASE_NAME}-osx-aarch64-fastdebug-b${build_number}
    ;;
  *)
    echo "***ERR*** bundle was not specified" && do_exit 1
    ;;
...
sh configure \
  --disable-warnings-as-errors --disable-hotspot-gtest --disable-javac-server --disable-full-docs --disable-manpages \
  $WITH_DEBUG_LEVEL \
...
if [ "$bundle_type" == "jcef" ]; then
  ...
  JBRSDK_TEST=$JBRSDK_BASE_NAME-osx-test-aarch64-b$build_number
